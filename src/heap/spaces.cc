...
FreeList* FreeList::CreateFreeList() {
  switch (FLAG_gc_freelist_strategy) {
    ...
    case 29: return new FreeListManyMoreLoopSelect();
    case 30: return new FreeListManyMoreLoopSelectLarge();
    case 31: return new FreeListHalfSmallMany();
    case 32: return new FreeListHalfMany();
    case 33: return new FreeListHalfSmallManyCachedFastPath();
    case 34: return new FreeListHalfManyCachedFastPath();
    case 35: return new FreeListManyCachedFastPath256();
    case 36: return new FreeListHalfSmallManyCachedFastPath256();
    case 37: return new FreeListHalfManyCachedFastPath256();
    case 38: return new FreeListManyFastPath();
    case 39: return new FreeListHalfManyFastPath();
    case 40: return new FreeListManyOrigin();
    case 41: return new FreeListManyMore2kFastPathNoCache();
    case 42: return new FreeListManyFastPathNoFallBackFast();
    case 43: return new FreeListLegacySlowPath();
    default:
...
FreeSpace FreeListLegacySlowPath::Allocate(size_t size_in_bytes, size_t* node_size, AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
  FreeSpace node;
...
// ------------------------------------------------
// FreeListManyFastPath implementation

FreeSpace FreeListManyFastPath::Allocate(size_t size_in_bytes,
                                         size_t* node_size,
                                         AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
FreeSpace FreeListManyFastPathNoFallBackFast::Allocate(size_t size_in_bytes,
                                                       size_t* node_size,
                                                       AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
// ------------------------------------------------
// FreeListManyCachedFastPath256 implementation

FreeSpace FreeListManyCachedFastPath256::Allocate(size_t size_in_bytes,
                                               size_t* node_size,
                                               AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
// ------------------------------------------------
// FreeListHalfSmallManyCachedFastPath implementation

FreeSpace FreeListHalfSmallManyCachedFastPath::Allocate(size_t size_in_bytes,
                                               size_t* node_size,
                                               AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
// ------------------------------------------------
// FreeListHalfSmallManyCachedFastPath256 implementation

FreeSpace FreeListHalfSmallManyCachedFastPath256::Allocate(size_t size_in_bytes,
                                               size_t* node_size,
                                               AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
FreeSpace FreeListHalfManyCachedFastPath::Allocate(size_t size_in_bytes,
                                               size_t* node_size,
                                               AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
FreeSpace FreeListHalfManyFastPath::Allocate(size_t size_in_bytes,
                                             size_t* node_size,
                                             AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
// ------------------------------------------------
// FreeListHalfManyCachedFastPath256 implementation

FreeSpace FreeListHalfManyCachedFastPath256::Allocate(size_t size_in_bytes,
                                                      size_t* node_size,
                                                      AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
...
FreeSpace FreeListManyOrigin::Allocate(size_t size_in_bytes,
                                             size_t* node_size,
                                             AllocationOrigin origin) {
  if (origin == AllocationOrigin::kGC) {
    return FreeListMany::Allocate(size_in_bytes, node_size, origin);
  } else {
    return FreeListManyFastPath::Allocate(size_in_bytes, node_size,
                                          origin);
  }
}
...
// ------------------------------------------------
// FreeListManyMore2kFastPathNoCache implementation

FreeSpace FreeListManyMore2kFastPathNoCache::Allocate(size_t size_in_bytes,
                                                    size_t* node_size,
                                                    AllocationOrigin origin) {
  USE(origin);
  DCHECK_GE(kMaxBlockSize, size_in_bytes);
