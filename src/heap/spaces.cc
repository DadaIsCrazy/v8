...
FreeList* FreeList::CreateFreeList() {
  if (FLAG_gc_experiment_fast_alloc) {
    return new FreeListFastAlloc();
  } else if (FLAG_gc_experiment_many_freelists) {
...
size_t FreeListMany::Free(Address start, size_t size_in_bytes,
                            FreeMode mode) {
  Page* page = Page::FromAddress(start);
  page->DecreaseAllocatedBytes(size_in_bytes);
